...
const simpleRateLimiter = (req, res, next) => {
    try {
        ...
        if (timestamps.length >= RATE_LIMIT_MAX_REQUESTS) {
            return res
                .status(429)
                .json(new ResponseDTO(null, { message: "تعداد درخواست‌ها بیش از حد مجاز است" }, false));
        }
        timestamps.push(now);
...
class AnalyticsController {
    ...
    async resolveOptionalUser(req) {
        try {
            ...
            if (!tokenUser)
                return undefined;
            return ((await userService.findUserByPhoneNumber(tokenUser.phoneNumber)) ||
                undefined);
        }
        catch {
            return undefined;
    ...
    async dashboard(req, res) {
        try {
            ...
            const [eventCounts, funnel, salesOverTime, mostViewedProducts, abandonedCartProducts, topSearchQueries,] = await Promise.all([
            ...
            const conversionRate = productViews > 0
                ...
                : 0;
            return res.status(200).json(new ResponseDTO(null, null, true, null, {
                rangeDays: days,
                totals: {
                    productViews,
