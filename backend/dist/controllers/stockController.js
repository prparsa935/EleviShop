...
class StockController {
    ...
    async adjustStock(req, res, next) {
        try {
            ...
            const movement = await stockMovementService.recordMovement(adjustDto.inventoryId, MovementType.MANUAL_ADJUSTMENT, adjustDto.quantityChange, adjustDto.reason, user.id);
            return res
                .status(200)
                .json(new ResponseDTO(null, null, true, "موجودی با موفقیت اصلاح شد", instanceToPlain(movement)));
        }
        catch (error) {
            next(error);
        }
    ...
    async getSlowMoving(req, res, next) {
        try {
            ...
            const salesThreshold = req.query.threshold
                ? Number(req.query.threshold)
                : 1;
            const items = await stockMovementService.getSlowMovingItems(dateRangeInMonths, salesThreshold);
            return res
                .status(200)
                .json(new ResponseDTO(null, null, true, null, instanceToPlain(items)));
        }
