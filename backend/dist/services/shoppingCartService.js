...
class ShoppingCartService {
    ...
    async getUserCart(userId) {
        ...
        for (const item of items) {
            if (item.itemType === CartItemType.SET) {
                ...
                try {
                    setComponents = await productSetService.resolveSetInventoriesForColor(item.productSet.id, item.color?.id);
                    const priceInfo = await productSetService.getFinalSetPrice(item.productSet.id, item.color?.id);
                    price = priceInfo.price;
                    isManual = priceInfo.isManual;
                    calculatedPrice = priceInfo.calculatedPrice;
                    availableQuantity = await productSetService.getAvailableSetQuantity(item.productSet.id, item.color?.id);
                }
                catch (error) {
                    setComponents = [];
                }
    ...
    async addItemToUserShopingCart(inventoryId, count, user, options) {
        ...
        if (itemType === CartItemType.SET) {
            ...
            const productSet = await productSetService.findProductSetById(productSetId);
            if (!productSet) {
                throw new OverallError("سرویس مورد نظر یافت نشد", 404);
            }
            const { colors, hasNoCommonColor } = await productSetService.getAvailableColorsForSet(productSetId);
            if (hasNoCommonColor || colors.length === 0) {
                throw new OverallError("این سرویس رنگ مشترکی بین قطعاتش ندارد و قابل سفارش نیست", 400);
            }
            const colorOption = colors.find((c) => c.colorId === colorId);
            if (!colorOption) {
                throw new OverallError("رنگ انتخابی جزو رنگ‌های معتبر این سرویس نیست", 400);
            }
            ...
            newShoppingCartItem.color = { id: colorId };
    ...
    async syncShoppingCart(user, items, replace = false) {
        ...
        (items ?? []).forEach((raw) => {
            ...
            if (isSet) {
                ...
                desiredSimple.set(inventoryId, existingCount !== undefined
        ...
        for (const [key, entry] of Array.from(desiredSets.entries())) {
            try {
                const { colors, hasNoCommonColor } = await productSetService.getAvailableColorsForSet(entry.productSetId);
                if (hasNoCommonColor || colors.length === 0) {
        ...
        for (const [key, entry] of desiredSets.entries()) {
            ...
            if (existingItem) {
                ...
                newShoppingCartItem.color = { id: entry.colorId };
                newShoppingCartItem.inventory = null;
                await this.shoppingCartRepo.save(newShoppingCartItem);
                dbItemMap.set(key, newShoppingCartItem);
            }
