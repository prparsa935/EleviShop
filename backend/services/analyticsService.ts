...
class AnalyticsService {
  private eventRepo = dataSource.getRepository(AnalyticsEvent);
   ...
   */
  async trackEvent(eventData: TrackEventData, user?: User): Promise<void> {
    try {
      ...
      console.error("analytics trackEvent failed:", (error as Error)?.message);
    }
  }
  ...
  async getConversionFunnel(dateRange: AnalyticsDateRange = {}) {
  ...
  async getSalesOverTime(
    ...
    const qb = this.orderRepo
      ...
      .addSelect(
        "COALESCE(SUM(oi.singleProductPrice * (100 - oi.singleProductOffPercent) / 100.0 * oi.quantity), 0)",
        "totalSales"
      )
  ...
  async getAbandonedCartProducts(limit = 10) {
    ...
    const purchasedMap = new Map(
      purchaseRows.map((row) => [Number(row.productId), Number(row.purchasedCount)])
    );
  ...
  async getTopSearchQueries(dateRange: AnalyticsDateRange = {}, limit = 10) {
    const rows = await this.applyDateRange(
      this.eventRepo
        .createQueryBuilder("event")
